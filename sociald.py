from sddata.mylib import config, thread
from sddata.mylib.mailer import Mailer
from sddata.mylib.detection import detect_people
...
inputv = "final/static/uploads/sociald/video/"
display = 1
output = "final/static/uploads/sociald/video/output/"

def gen(filename,source):
	...
	# otherwise, grab a reference to the video file
		...
		vs = cv2.VideoCapture(inputv+filename)
		if config.Thread:
				cap = thread.ThreadingClass(inputv+filename)
	...
	#------------------------------------------------------------------------------#
		...
		if display > 0:
			...
			vframe = cv2.imencode('.jpg', frame)[1].tobytes()
			yield (b'--frame\r\n'
		...
		if output != "" and writer is None:
			...
			writer = cv2.VideoWriter(output+filename, fourcc, 25,
